...
export function TransactionModal({ onClose, onSuccess, initialSubtype, initialTxType, initialStep = 0, skipAutoPrint = false }: Props) {
  ...
  // Default: só o mês atual. Operador escolhe outros meses manualmente na lista.
  useEffect(() => {
    if (txType !== 'income' || incomeSubtype !== 'mensalidade' || isAcordo || !resident) return
    const now = new Date()
    setMensalidadeMonths([`${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`])
  }, [incomeSubtype, txType, isAcordo, resident])
              ...
              {!isProof && (
                <>
                  ...
                  {txType === 'income' && incomeSubtype === 'mensalidade' && !isAcordo && resident && (() => {
                    ...
                    const paidSet = new Set(residentMensalidades.filter(m => m.status === 'paid').map(m => m.reference_month))
                    const months: string[] = []
                    const start = new Date(2025, 3, 1) // abril/2025 — início do controle de mensalidades
