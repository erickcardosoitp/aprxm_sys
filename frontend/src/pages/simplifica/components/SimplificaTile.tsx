...
export function SimplificaTile({ icon: Icon, label, onClick, badge, color = '#1a3f6f' }: Props) {
  return (
    <button
      onClick={onClick}
      className="group relative rounded-2xl flex flex-col items-start justify-between bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 active:scale-[0.96] transition-transform duration-100 w-full h-full"
      style={{
        padding: '14px',
    ...
    >
      ...
      <span className="text-[13px] font-semibold text-left text-gray-800 dark:text-gray-100 leading-tight tracking-tight w-full mt-2">
        {label}
      </span>
    </button>
  )
