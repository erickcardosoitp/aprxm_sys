...
const MODULES = ['residents', 'packages', 'service_orders', 'finance', 'admin', 'settings']
const MODULE_LABEL: Record<string, string> = {
  residents: 'Moradores', packages: 'Encomendas', service_orders: 'Ordens', finance: 'Financeiro', admin: 'Admin', settings: 'Config',
}
  ...
  return (
    <div className="px-6 py-4 overflow-auto" style={{ fontFamily: "'IBM Plex Sans', sans-serif" }}>
      <p className="text-xs mb-4" style={{ color: TEXT_MUTED }}>
        Template único da empresa — vale para todas as unidades. superadmin/admin_master têm acesso total.
      </p>
      {MODULES.map((mod) => (
            ...
            <tbody>
              {ROLES.map((role) => (
                <tr key={role} className="border-b" style={{ borderColor: BORDER }}>
                  <td className="py-1.5 pr-4">{role}</td>
                  {PERMS.map((p) => (
