...
async def create_demand(
    ...
    row = (await session.execute(text("""
        INSERT INTO demands (association_id, title, description, status, phase, priority,
                             assigned_to, assigned_to_name, due_date, notes, created_by)
        VALUES (:aid, :title, :desc, :status, :phase, :priority,
                :at, :atn, :dd, :notes, :cb)
        RETURNING id, title, status, phase, priority, assigned_to_name, due_date, created_at
    """), {
    ...
    cols = ["id","title","status","phase","priority","assigned_to_name","due_date","created_at"]
...
async def update_demand(
    ...
    row = (await session.execute(text(f"""
        ...
        RETURNING id, title, description, status, phase, priority,
                  assigned_to, assigned_to_name, due_date, notes, updated_at
    """), p)).fetchone()
    if not row:
