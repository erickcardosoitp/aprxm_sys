...
Rodam automaticamente no startup da aplicacao (ver `lifespan` em app/main.py) —
nao ha comando manual de migration. O controle e' a constante SCHEMA_VERSION
abaixo, comparada com MAX(version) na tabela `schema_migrations`.

Contexto: o deploy e' serverless (Vercel), onde nao existe um passo de release
pra rodar migration antes de subir o codigo — por isso elas vivem no startup,
protegidas por advisory lock (uma instancia migra, as outras esperam) e por um
fast-exit de ~2ms quando o schema ja esta na versao atual.

COMO ADICIONAR UMA MIGRATION:
  1. incremente SCHEMA_VERSION
  2. adicione o bloco no FIM de _apply_versioned_migrations, isolado em
     try/except com rollback (uma falha nao pode derrubar a app inteira)
  3. use sempre DDL aditivo e idempotente (IF NOT EXISTS / ADD COLUMN IF NOT
     EXISTS) — o bloco pode reexecutar num cold start concorrente
...
async def _create_base_schema(session) -> None:
    ...
    Caminho raro: no docker-compose e em producao o banco ja nasce com as tabelas
    (database/schema.sql / DB existente), entao a deteccao manda direto pras
    versionadas. Mantido pra quem aponta pra um Postgres cru."""
...
async def _apply_versioned_migrations(session) -> None:
    """Blocos v2..vN. Rodam nos DOIS caminhos (banco novo e banco existente).

    Antes existiam duplicados nos dois ramos do if — foi assim que a v4 sumiu do
    caminho de banco novo. Agora e um lugar so: cada migration se escreve uma vez."""
...
async def _assert_schema_bootstrapped() -> None:
    """Falha cedo e com mensagem util se o banco estiver completamente vazio.

    As migrations sao todas incrementais (ALTER TABLE / ADD COLUMN) e pressupoem um
    schema ja existente. Varias tabelas (cash_boxes, chat_messages, notifications...)
    nem tem model SQLModel — vivem so em database/schema.sql. Entao um Postgres cru
    nao tem como ser bootstrapado daqui; o schema.sql precisa ser aplicado antes.
    O docker-compose ja faz isso automaticamente."""
    async with AsyncSessionLocal() as s:
        is_empty = (await s.execute(text(
            "SELECT COUNT(*) = 0 FROM information_schema.tables WHERE table_schema = 'public'"
        ))).scalar()
    if is_empty:
        raise RuntimeError(
            "Banco vazio: aplique database/schema.sql antes de subir a aplicacao.\n"
            "  docker compose up      -> faz isso automaticamente\n"
            "  banco proprio          -> psql -d <db> -f database/schema.sql\n"
            "As migrations sao incrementais e nao criam o schema do zero."
        )


async def run_migrations() -> None:
    await _assert_schema_bootstrapped()
    ...
    async with AsyncSessionLocal() as session:
        ...
        if _is_existing_db:
            ...
            # Banco cru: cria as tabelas base antes das versionadas.
            await _create_base_schema(session)
            await session.execute(text(
                "INSERT INTO schema_migrations (version, description) "
        ...
        await _apply_versioned_migrations(session)
